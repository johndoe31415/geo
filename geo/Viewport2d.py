...
class Viewport2d(object):
	def __init__(self, device_width, device_height, logical_center_x = 0, logical_center_y = 0, logical_width = 1, logical_height = 1, keep_aspect_ratio = False):
		...
		if self._keep_aspect_ratio:
			logical_width = logical_height * device_width / device_height
	...
	def export_pos(self):
		return {
			"center":	list(self._logical_center),
			"size":		list(self._logical_size),
		}

	def import_pos(self, position):
		self._logical_center = Vector2d(position["center"][0], position["center"][1])
		self._logical_size = Vector2d(position["size"][0], position["size"][1])
